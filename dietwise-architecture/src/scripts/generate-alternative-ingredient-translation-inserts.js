...
// node dietwise-architecture/src/scripts/generate-alternative-ingredient-translation-inserts.js LT < dietwise-architecture/src/translations/AltIngredients_LT.csv

const fs = require('node:fs');
const path = require('node:path');

const EXPECTED_COLUMNS = 2;
const SOURCE_TABLE = 'DW_ALTERNATIVE_INGREDIENT';
const TRANSLATION_TABLE = 'DW_ALTERNATIVE_INGREDIENT_TRANSLATION';
const HEADER_FIRST_COLUMN = 'name of alternative';
...
function readAlternativeIngredientIds(changelogsDir) {
...
function requireId(map, name, lineNo) {
	...
	if (!id) {
		throw new Error(`Line ${lineNo}: Cannot resolve alternative ingredient name "${name}" to UUID from existing changelog inserts.`);
	}
	return id;
}
...
function main() {
	...
	if (!lang) {
		throw new Error('Usage: generate-alternative-ingredient-translation-inserts.js <lang> < input.csv');
	}
	...
	const alternativeIngredientIds = readAlternativeIngredientIds(changelogsDir);
	...
	for (let i = 0; i < dataRows.length; i++) {
		...
		const alternativeIngredientName = fields[0];
		const translatedName = fields[1];

		const alternativeIngredientId = requireId(alternativeIngredientIds, alternativeIngredientName, lineNo);

		out.push(`\t\t<insert tableName="${TRANSLATION_TABLE}">`);
		out.push(`\t\t\t<column name="alternative_ingredient_id" value="${alternativeIngredientId}"/>`);
