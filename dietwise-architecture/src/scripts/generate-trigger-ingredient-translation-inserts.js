...
// node dietwise-architecture/src/scripts/generate-trigger-ingredient-translation-inserts.js LT < dietwise-architecture/src/translations/TriggerIngredients_LT.csv

const fs = require('node:fs');
const path = require('node:path');

const EXPECTED_COLUMNS = 4;
const SOURCE_TABLE = 'DW_TRIGGER_INGREDIENT';
const TRANSLATION_TABLE = 'DW_TRIGGER_INGREDIENT_TRANSLATION';
const HEADER_FIRST_COLUMN = 'trigger ingredient';
...
function readTriggerIngredientIds(changelogsDir) {
...
function requireId(map, name, lineNo) {
	...
	if (!id) {
		throw new Error(`Line ${lineNo}: Cannot resolve trigger ingredient name "${name}" to UUID from existing changelog inserts.`);
	}
	return id;
}
...
function main() {
	...
	if (!lang) {
		throw new Error('Usage: generate-trigger-ingredient-translation-inserts.js <lang> < input.csv');
	}
	...
	const triggerIngredientIds = readTriggerIngredientIds(changelogsDir);
	...
	for (let i = 0; i < dataRows.length; i++) {
		...
		const triggerIngredientName = fields[0];
		const translatedName = fields[2];
		const translatedExplanation = fields[3];

		const triggerIngredientId = requireId(triggerIngredientIds, triggerIngredientName, lineNo);

		out.push(`\t\t<insert tableName="${TRANSLATION_TABLE}">`);
		out.push(`\t\t\t<column name="trigger_ingredient_id" value="${triggerIngredientId}"/>`);
		out.push(`\t\t\t<column name="lang" value="${xmlEscape(lang)}"/>`);
		out.push(valueColumn('name', translatedName));
		out.push(valueColumn('explanation_for_llm', translatedExplanation));
		out.push(`\t\t</insert>`);
