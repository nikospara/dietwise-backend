...
// node dietwise-architecture/src/scripts/generate-role-or-technique-translation-inserts.js LT < dietwise-architecture/src/translations/RolesTechniques_LT.csv

const fs = require('node:fs');
const path = require('node:path');

const EXPECTED_COLUMNS = 4;
const SOURCE_TABLE = 'DW_ROLE_OR_TECHNIQUE';
const TRANSLATION_TABLE = 'DW_ROLE_OR_TECHNIQUE_TRANSLATION';
const HEADER_FIRST_COLUMN = 'name';
...
function readRoleOrTechniqueIds(changelogsDir) {
...
function requireId(map, name, lineNo) {
	...
	if (!id) {
		throw new Error(`Line ${lineNo}: Cannot resolve role or technique name "${name}" to UUID from existing changelog inserts.`);
	}
	return id;
}
...
function main() {
	...
	if (!lang) {
		throw new Error('Usage: generate-role-or-technique-translation-inserts.js <lang> < input.csv');
	}
	...
	const roleOrTechniqueIds = readRoleOrTechniqueIds(changelogsDir);
	...
	for (let i = 0; i < dataRows.length; i++) {
		...
		const roleOrTechniqueName = fields[0];
		const translatedName = fields[2];
		const translatedExplanation = fields[3];

		const roleOrTechniqueId = requireId(roleOrTechniqueIds, roleOrTechniqueName, lineNo);

		out.push(`\t\t<insert tableName="${TRANSLATION_TABLE}">`);
		out.push(`\t\t\t<column name="role_or_technique_id" value="${roleOrTechniqueId}"/>`);
