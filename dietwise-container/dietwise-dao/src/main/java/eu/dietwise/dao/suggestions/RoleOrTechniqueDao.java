...
public interface RoleOrTechniqueDao {
	...
	/**
	 * Revert a Role or Technique's staged edit, restoring its published master name and explanation and removing the
	 ...
	 */
	Uni<Void> revertRoleOrTechnique(ReactivePersistenceTxContext tx, UUID id, long baseVersion);

	/**
	 * For each Role or Technique with at least one translation (published or staged), which languages are translated in
	 * published master and which carry a pending change in the Working Copy. Keyed by Role or Technique id; sparse — an
	...
	/**
	 * Stage a Role or Technique's name and explanation translation for one language in the Working Copy, leaving
